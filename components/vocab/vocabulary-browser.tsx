...
import { Search, BookOpen, Zap, Target } from 'lucide-react'
    ...
    <div className="space-y-6">
        ...
        <div className="flex gap-2 flex-wrap">
          {['all', 'Beginner', 'Intermediate', 'Advanced'].map((level) => (
            <Button
              ...
              className={selectedDifficulty === level ? 'bg-primary text-primary-foreground' : ''}
            >
              {level}
            </Button>
          ))}
      ...
      {activeTab === 'my-sets' && (
        <div className="text-center py-12">
          <BookOpen className="mx-auto h-12 w-12 text-foreground/20 mb-4" />
          <p className="text-foreground/60">No custom sets yet. Create your first set to get started!</p>
          <Button className="mt-4 bg-primary text-primary-foreground hover:bg-primary/90">Create Set</Button>
        </div>
      )}

      {activeTab === 'challenges' && (
        <div className="text-center py-12">
          <Target className="mx-auto h-12 w-12 text-foreground/20 mb-4" />
          <p className="text-foreground/60">No active challenges. Check back daily!</p>
        </div>
      )}

      {activeTab === 'games' && (
        <div className="text-center py-12">
          <Zap className="mx-auto h-12 w-12 text-foreground/20 mb-4" />
          <p className="text-foreground/60">Games coming soon! Start studying to unlock them.</p>
        </div>
      )}
    </div>
  )
...
function VocabSetCard({ set }: { set: typeof VOCAB_CATEGORIES[0] }) {
  ...
  return (
    <Card className="border-border hover:border-primary transition-colors cursor-pointer overflow-hidden group">
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className={`px-2 py-1 rounded-full text-xs font-medium ${difficultyColor[set.difficulty]}`}>
            {set.difficulty}
          </span>
        </div>
