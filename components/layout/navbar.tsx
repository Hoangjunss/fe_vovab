...
import {
  Menu, X, BookOpen, Target, Zap, LayoutDashboard, Swords,
  LogIn, UserPlus, LogOut, User, Settings
} from 'lucide-react';
...
export function Navbar() {
  ...
  return (
    <>
        ...
        <div className="container mx-auto flex h-16 items-center justify-between px-4 md:px-6">
          ...
          <div className="hidden md:flex items-center gap-2">
            {user ? (
              ...
              <>
                <Button variant="ghost" size="sm" onClick={() => router.push('/login')} className="gap-1 text-foreground/70 hover:text-primary transition-all hover:scale-105">
                  <LogIn className="h-4 w-4" /> Đăng nhập
                </Button>
                <Button size="sm" onClick={() => router.push('/register')} className="bg-primary text-white hover:bg-primary/90 shadow-md transition-all hover:scale-105">
      ...
      {/* Mobile drawer (giữ nguyên CSS transition như cũ) */}
      <div className={`fixed inset-0 top-16 z-40 transition-all duration-300 md:hidden ${drawerOpen ? 'visible opacity-100' : 'invisible opacity-0'}`}>
        ...
        <div className={`absolute inset-x-0 top-0 bg-white/95 backdrop-blur-md shadow-xl border-t border-border/50 transition-all duration-300 transform ${drawerOpen ? 'translate-y-0' : '-translate-y-full'}`}>
...
function NavLink({ href, children, icon }: { href: string; children: React.ReactNode; icon: React.ReactNode }) {
  return (
    <Link href={href} className="group flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-muted-foreground hover:text-primary transition-all duration-200 rounded-full hover:bg-primary/10 active:scale-95">
      {icon}{children}
    </Link>
  );
}
...
function DrawerLink({ href, onClick, children, icon }: { href: string; onClick: () => void; children: React.ReactNode; icon?: React.ReactNode }) {
  return (
    <Link href={href} onClick={onClick} className="flex items-center gap-3 px-4 py-3 rounded-xl text-base font-medium text-foreground hover:bg-primary/10 hover:text-primary transition-all duration-200 active:scale-95">
      {icon && <span className="text-primary/70">{icon}</span>}{children}
    </Link>
  );
}